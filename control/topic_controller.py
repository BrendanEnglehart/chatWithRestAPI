...
class TopicController():
    ...
    def create_topic(self, name, topic_type, category_id, metadata=""):
        """Create a topic"""
        if self.dbconnection.count_documents({"name" : name,   "category_id" : category_id}) > 0:
            return False
    ...
    def general_landing(self, category_id):
        ...
        if self.dbconnection.count_documents({"name" : "general",   "category_id" : category_id}) > 0:
            return self.dbconnection.find_one({"name" : "general",   "category_id" : category_id})
        return self.create_topic(name="general", topic_type="general", category_id=category_id)
